...
| File | What it does |
|---|---|
| `baseline-snapshot.sh` | Writes 39 text files (identity, driver, packages, holds, accounts and service accounts, sshd config, network, storage, services, containers, host guard config) plus `99-summary.txt` to `baseline/<host>/<timestamp>/`. Modifies nothing. Root not required: `33-sshd-effective`, `35-sudoers` and the ufw rules in `45-ufw` need root and are skipped with a note when `sudo -n` would prompt. |
...
`36-service-accounts.txt` lists `svc-*` accounts, which `30-users.txt` misses because `useradd --system` puts them below uid 1000. `67-guard-config.txt` records the thresholds and the resolved unit of the [gb10-ops](https://github.com/ivanusto/gb10-ops) host memory guard, so a threshold change is a diff like any other; it reads `[absent]` on a node without the guard.
